(function(){
  ...
  function interactionPanel(id){var p=POSTS[id];return '<aside class="blog-interactions" id="discussion" data-post-panel="'+id+'">'+
    '<div class="article-actions">'+pdfButton(id)+reactionButtons(id,false)+shareBlock(id)+'</div>'+
  ...
  function postIdFromPath(path){var m=String(path||location.pathname).match(/^\/blog\/([^/?#]+?)(?:\.html)?\/?$/);return m&&POSTS[m[1]]?m[1]:null;}
  function installIndividual(){var id=postIdFromPath();if(!id)return;var meta=document.querySelector('.papermeta');if(meta&&!document.querySelector('.top-post-actions'))meta.insertAdjacentHTML('afterend','<div class="top-post-actions">'+pdfButton(id)+reactionButtons(id,false)+shareBlock(id)+'</div>');var aidisc=document.querySelector('.aidisc');if(!aidisc)return;aidisc.insertAdjacentHTML('beforebegin',interactionPanel(id));}
  function installPreviews(){document.querySelectorAll('#bf article.post,#bt article.post').forEach(function(card){var link=card.querySelector('.plink');var id=link&&postIdFromPath(link.getAttribute('href'));if(!id||card.querySelector('.preview-actions'))return;card.insertAdjacentHTML('beforeend','<div class="preview-actions"></div>');var actions=card.querySelector('.preview-actions');actions.appendChild(link);actions.insertAdjacentHTML('beforeend',pdfButton(id)+reactionButtons(id,true)+shareBlock(id));});}
  ...
  async function load(ids){if(!client||!ids.length)return;var result=await client.rpc('get_blog_interactions',{p_post_ids:ids});if(result.error)return;result.data.forEach(function(row){state[row.post_id]={likes:Number(row.likes)||0,dislikes:Number(row.dislikes)||0,reaction:Number(row.user_reaction)||0,comments:Array.isArray(row.comments)?row.comments:[]};paint(row.post_id);renderComments(row.post_id);});}
  function paint(id){var s=state[id]||{likes:0,dislikes:0,reaction:0};document.querySelectorAll('[data-reactions="'+id+'"]').forEach(function(box){var like=box.querySelector('.react-like'),dislike=box.querySelector('.react-dislike');box.querySelector('.like-count').textContent=s.likes;box.querySelector('.dislike-count').textContent=s.dislikes;like.classList.toggle('selected',s.reaction===1);dislike.classList.toggle('selected',s.reaction===-1);like.setAttribute('aria-pressed',String(s.reaction===1));dislike.setAttribute('aria-pressed',String(s.reaction===-1));});}
  ...
  function setPdfHref(a){var entries=pdfEntries(POSTS[a.dataset.pdfPost]),entry=entries[Number(a.dataset.pdfIndex)||0];if(entry)a.href=localized(entry.url);}
  ...
  function bindShare(){document.addEventListener('click',function(e){var toggle=e.target.closest('[data-share-toggle]'),targetMenu=toggle&&toggle.closest('.share-wrap').querySelector('.share-menu');document.querySelectorAll('.share-menu.open').forEach(function(m){if(m!==targetMenu){m.classList.remove('open');var b=m.closest('.share-wrap').querySelector('[data-share-toggle]');if(b)b.setAttribute('aria-expanded','false');}});if(toggle){e.stopPropagation();var open=targetMenu.classList.toggle('open');toggle.setAttribute('aria-expanded',String(open));return;}var item=e.target.closest('[data-share]');if(!item)return;var menu=item.closest('.share-menu'),wrap=menu.closest('.share-wrap'),id=menu.dataset.shareMenu,url=new URL(wrap.dataset.shareUrl||('/blog/'+id),location.origin).href,title=wrap.dataset.shareTitle||document.title,type=item.dataset.share;if(type==='copy'){e.preventDefault();navigator.clipboard.writeText(url).then(function(){item.textContent=tx(TEXT.copied);});return;}var links={linkedin:'https://www.linkedin.com/sharing/share-offsite/?url='+encodeURIComponent(url),x:'https://twitter.com/intent/tweet?url='+encodeURIComponent(url)+'&text='+encodeURIComponent(title),facebook:'https://www.facebook.com/sharer/sharer.php?u='+encodeURIComponent(url),whatsapp:'https://wa.me/?text='+encodeURIComponent(title+' '+url),email:'mailto:?subject='+encodeURIComponent(title)+'&body='+encodeURIComponent(url)};item.href=links[type];item.target=type==='email'?'':'_blank';item.rel='noopener';});}
  function observeLanguage(){new MutationObserver(function(){document.querySelectorAll('[data-comment-form] input[name="display_name"]').forEach(function(el){el.placeholder=tx(TEXT.name);});document.querySelectorAll('[data-comment-form] textarea').forEach(function(el){el.placeholder=tx(TEXT.comment);});document.querySelectorAll('.react-like').forEach(function(el){el.setAttribute('aria-label',tx(TEXT.like));});document.querySelectorAll('.react-dislike').forEach(function(el){el.setAttribute('aria-label',tx(TEXT.dislike));});var close=document.querySelector('.captcha-close');if(close)close.setAttribute('aria-label',tx(TEXT.close));document.querySelectorAll('[data-pdf-post]').forEach(setPdfHref);document.querySelectorAll('[data-comments]').forEach(function(el){renderComments(el.dataset.comments);});}).observe(document.documentElement,{attributes:true,attributeFilter:['data-lang']});}
  ...
  function init(){installIndividual();installPreviews();installPublications();installMaterials();installGate();initClient();bindReactions();bindComments();bindPdfLanguage();bindPdfMenu();bindShare();observeLanguage();var ids=Array.from(document.querySelectorAll('[data-reactions]')).map(function(el){return el.dataset.reactions;}).filter(function(id,i,all){return all.indexOf(id)===i;});load(ids);}

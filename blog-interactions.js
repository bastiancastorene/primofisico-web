(function(){
  ...
  function shareTitle(element){var current=document.documentElement.getAttribute('data-lang')||'es',localized=element&&element.querySelector('.lang-'+current);return String((localized||element||{}).textContent||document.title).trim();}
  function installIndividual(){var id=postIdFromPath();if(!id)return null;if(id.indexOf('saber-')===0){var byline=document.querySelector('.pf-saber-byline'),title=document.querySelector('.pf-saber-article h1');if(byline&&!document.querySelector('.saber-top-actions'))byline.insertAdjacentHTML('afterend','<div class="top-post-actions saber-top-actions">'+reactionButtons(id,false)+viewCount(id,false)+shareBlock(id,location.pathname,shareTitle(title))+'</div>');return id;}var meta=document.querySelector('.papermeta');if(meta&&!document.querySelector('.top-post-actions'))meta.insertAdjacentHTML('afterend','<div class="top-post-actions">'+pdfButton(id)+reactionButtons(id,false)+viewCount(id,false)+shareBlock(id)+'</div>');var aidisc=document.querySelector('.aidisc');if(aidisc)aidisc.insertAdjacentHTML('beforebegin',interactionPanel(id));return id;}
  function installPreviews(){document.querySelectorAll('#bf article.post,#bt article.post,#saber-feed article.pf-saber-post-card').forEach(function(card){var link=card.querySelector('.plink');var id=link&&postIdFromPath(link.getAttribute('href'));if(!id||card.querySelector('.preview-actions'))return;card.insertAdjacentHTML('beforeend','<div class="preview-actions"></div>');var actions=card.querySelector('.preview-actions');actions.appendChild(link);actions.insertAdjacentHTML('beforeend',pdfButton(id)+reactionButtons(id,true)+viewCount(id,true)+shareBlock(id,link.getAttribute('href'),shareTitle(card.querySelector('h3'))));});}
